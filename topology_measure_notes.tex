...
\begin{proof}
    ...
    We now show that the characteristic property uniquely determines a structure on $X$. First let $\calE$ be a structure on $X$ such that a map $f \colon Y \to X$ is a homomorphism if and only if each $f_\alpha \circ f$ is a homomorphism. The commutative diagram
    %
    \begin{equation*}
        \begin{tikzcd}
            & X_\alpha \\
